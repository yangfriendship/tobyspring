...
# 8장 스프링이란 무엇인가?
## 8.1 스프링의 정의
자바 앤터프라이즈 개발을 편하게 해주는 오픈소스 경량급 어플리케이션 프레임워크
1. 어플리케이션
특정 계층이나 기술 업무 분야에 국한되지 않고 어플리케이션의 전 영역을 포괄하는 범용적인 프레임워크!
    - "항상 프레임워크 기반으로 접근하라"
    - 전 영역을 관통화는 일관적인 모델과 핵심기술을 바탕으로 각 분와야 특성에 맞게 채워주기 때문에 어플리케이션을 쉽고 빨게 개발할 수 있다.
2. 경량급
    - 스프링에서 `경량`의 의미 : 불필요하지 않게 무겁지 않다.
    - EJB와 비교하자면 고가의 서버와 개발환경이 없이도 개발이 가능하다.
3. 자바 엔터프라이즈 개발을 편하게
    - 개발자가 복잡하고 실수하기 쉬운 로우레벨 기술에 많은 신경을 쓰지않고, 비지니스 로직을 빠르고 효과적으로 개발하는 것
    - 엔터프라이즈 개발에서 필요한 부분을 기술적인 복잡함을 줄여주면서 개발을 도와준다.
4. 오픈소스
    - 스프링은 아파치 라이센스 ver.2
    - 상업적 프로젝트에 이용가능, 비공개 프로젝트에도 이용가능!
    - 오픈소스는 상업 프로그램과 비교해 수정,개발 등이 느리지만 스프링은 `SpringSource`(전문기업)에서 전적으로 관리하기 때문에 변화에 빠르게 대응한다.
    - 오픈소스의 장점을 취하면서 단점도 극복하고 있는 엄청난 오픈소스 소프트웨어!

## 8.2 스프링의 목적
언어와 라이브러리는 그 편리한 도구, 개발을 잘하려면 근본적으로 프로그래밍 실력이 필요하다.

## 8.2.1 엔터프라이즈 개발의 복잡함
엔터프라이즈 개발은 복잡하다. 2000년 대 초반에는 80% 엔터프레이즈 개발 프로젝트가 실패했다(예산,시간문제)
### 복잡함의근본적인이유
1. `기술적인 제약조건`과 `요구사항`이 증가
    - 엔터프라이즈 시스템을 개발하는데는 `순수 비지니스 로직`을 제외하고도 기술적으로 고려할 부분이 많다.(로우레벨)
2.  엔터프라이즈 어플리케이션이 구현해야 할 핵심기능인 비즈니스 로직의 복잡함이 증가
    - 시스템 개발과 유지보수， 추가 개발 등의 작업에 대한 부담은 커짐에 따라 개발의 난이도가 증가했다.